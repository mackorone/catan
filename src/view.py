...
def get_tile_grid(tile, template):
    ...
    tile_grid = replace_corners(tile, tile_grid)
    tile_grid = replace_edges(tile, tile_grid)
    tile_grid = replace_numbers(tile, tile_grid)
    tile_grid = replace_resources(tile, tile_grid)
    return tile_grid


def replace_corners(tile, tile_grid):
    tile_grid = copy_grid(tile_grid)
    for row, col in CORNER_SPACES:
        colored = Color.GRAY.apply(tile_grid[row][col])
        tile_grid[row][col] = colored
    return tile_grid


def replace_edges(tile, tile_grid):
    tile_grid = copy_grid(tile_grid)
    for row, col in EDGE_SPACES:
        colored = Color.GRAY.apply(tile_grid[row][col])
        tile_grid[row][col] = colored
    return tile_grid
    ...
    def get_board_grid(self):
        ...
        for group in [
            ...
            for coordinate, tile in group.items():
                ...
                copy = copy_grid(TILE_TEMPLATE)
                if group == self.board.border_tiles:
                    for neighbor in coordinate.get_all_neighbors():
                        if not neighbor in self.board.border_tiles:
                            continue
                        if coordinate.row == neighbor.row:
                            if coordinate.column < neighbor.column:
                                copy[3][8] = ' '
                        ...
                        if coordinate.column == neighbor.column:
                            if coordinate.row < neighbor.row:
                                copy[3][1] = ' '
                            if neighbor.row < coordinate.row:
                                copy[1][8] = ' '
                        if abs(diff) == abs(neighbor.row - neighbor.column):
                            if coordinate.column < neighbor.column:
                                copy[4][4] = ' '
                                copy[4][5] = ' '
                            if neighbor.column < coordinate.column:
